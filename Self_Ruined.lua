...
local UpdatingFunctions = {
	ESP = function(Entry, TopTextObject, BottomTextObject)
		...
		if GetRenderProperty(TopTextObject, "Visible") then
			...
			local Content, Player, IsAPlayer = "", Entry.Object, Entry:IsA('Player')
	...
	HeadDot = function(Entry, CircleObject, CircleOutlineObject)
		...
		local Character = Entry:IsA('Player') and __index(Entry.Object, "Character") or __index(Entry.Object, "Parent")
...
local CreatingFunctions = {
	...
	HeadDot = function(Entry)
		...
		if not Entry:IsA('Player') and not Entry.PartHasCharacter then
	...
	HealthBar = function(Entry)
		...
		if not Entry:IsA('Player') and not Humanoid then
			return
		end
	...
	Chams = function(Entry)
		...
		elseif not Entry:IsA('Player') then
			xpcall(function()
		...
		Entry.Connections.Chams = Connect(__index(RunService, Environment.DeveloperSettings.UpdateMode), function()
			for Index, Value in next, ChamsEntry do
				local Character = Entry:IsA('Player') and __index(Object, "Character") or __index(Object, "Parent")
...
local UtilityFunctions = {
	InitChecks = function(self, Entry)
		if not Entry:IsA('Player') and not Entry.PartHasCharacter and not Entry.RenderDistance then
			return
		end
		...
		local IsAPlayer = Entry:IsA('Player')
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		...
		repeat wait(0) until Entry:IsA('Player') and FindFirstChildOfClass(__index(Entry.Object, "Character"), "Humanoid") or true

		if not Entry:IsA('Player') then
		...
		local Humanoid = Entry:IsA('Player') and FindFirstChildOfClass(__index(Entry.Object, "Character"), "Humanoid") or FindFirstChildOfClass(__index(Entry.Object, "Parent"), "Humanoid")

		Entry.PartHasCharacter = not Entry:IsA('Player') and Humanoid
		Entry.RigType = Humanoid and (__index(Humanoid, "RigType") == 0 and "R6" or "R15") or "N/A"
		...
		Entry.Connections.PlayerUnwrapSignal = Connect(Entry.Object.Changed, function(Property)
			if DeveloperSettings.UnwrapOnCharacterAbsence and Property == "Parent" and not IsDescendantOf(__index(Entry.Object, (Entry:IsA('Player') and "Character" or Property)), Workspace) then
				self.UnwrapObject(nil, Entry.Hash)
			end
		end)
