...
local Window = AtomLibrary:StartWindow({
    Title = 'Ass',
    CloseBind = Enum.KeyCode.Insert
})

local Tab = Window:PlaceTab('TabName')
local elsE = Window:PlaceTab('Else')

local dd = Tab:MakeDropdown({
    Title = 'Ass',
    Items = {'theres no way', 'thers yes way'},
    Callback = function(v)
        print(v)
    end
})
